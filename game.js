...
function display() {
	"use strict";
	var i, j, id, str, g = document.getElementById("gameover");
	str = "";
	for (i = 0; i < 4; i += 1) {
		for (j = 0; j < 4; j += 1) {
			id = "" + i + j;
			if (game.grid[i][j] === 0) {
				...
				document.getElementById(id).style.background = "rgba(238, 228, 218, 0.35)";
			} else {
...
document.onkeydown = function (event) {
	...
	switch (key) {
	case 37:
		left(game); // execute a function by passing parameter 
		break;
	case 38:
		up(game);
		break;
...
window.onload = function () {
	...
	if (typeof (Storage) !== "undefined" && localStorage.getItem("gameState") !== null && localStorage.getItem("score") !== null) {
